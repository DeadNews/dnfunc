...
from vapoursynth import GRAY, YUV, VideoNode, core  # pylint: disable=no-name-in-module
...
def insane_aa(
    ...
    faster_aa: bool = False,
    eedi3_mode: Union[EEDI3Mode, Tuple[EEDI3Mode, EEDI3Mode]] = EEDI3Mode.CPU,
    eedi3_device: Union[int, Tuple[int, int]] = -1,
    eedi3_opt: Union[int, Tuple[int, int]] = 0,
    nnedi3_mode: Union[NNEDI3Mode, Tuple[NNEDI3Mode, NNEDI3Mode]] = NNEDI3Mode.NNEDI3,
    nnedi3_device: Union[int, Tuple[int, int]] = -1,
    nnedi3_opt: Union[int, str, Tuple[Union[int, str], Union[int, str]]] = 0,
    descale_strength: float = 0.3,
    kernel: str = "bilinear",
    bicubic_b: float = 1 / 3,
    bicubic_c: float = 1 / 3,
    lanczos_taps: int = 3,
    descale_width: int = None,
    descale_height: int = 720,
    pscrn: int = 1,
    alpha: float = 0.2,
    beta: float = 0.25,
    gamma: float = 1000.0,
    ...
    nns: int = 4,
    dehalo: bool = False,
    output_mode: ClipMode = ClipMode.FULL,
...
def revert_upscale(
    ...
    descale_mix = core.std.Expr(
        [descale_natural, descale_smooth],
        "x {strength} * y 1 {strength} - * +".format(strength=descale_strength),
    )
    if dehalo:
        descale_mix = fine_dehalo(
            descale_mix,
            darkstr=0,
            brightstr=1.5,
            thmi=45,
...
def rescale(
    ...
    if faster_aa:
        ...
        return core.resize.Spline36(
            clip, dx, dy, src_left=-0.5, src_top=-0.5, src_width=ux, src_height=uy
        )


def eedi3_instance(
    clip: VideoNode,
    eedi3_mode: EEDI3Mode = EEDI3Mode.CPU,
    eedi3_device: int = -1,
    eedi3_opt: int = 0,
    nnedi3_mode: NNEDI3Mode = NNEDI3Mode.NNEDI3,
    nnedi3_device: int = -1,
    nnedi3_opt: Union[int, str] = 0,
    pscrn: int = 1,
    alpha: float = 0.2,
    beta: float = 0.25,
    gamma: float = 1000.0,
    ...
    if eedi3_mode in [EEDI3Mode.OPENCL, 1, "opencl"]:
        return core.eedi3m.EEDI3CL(
            ...
            nrad=nrad,
            mdis=mdis,
            sclip=nnedi3_superclip(
                clip, nnedi3_mode, nnedi3_device, nnedi3_opt, pscrn, nsize, nns
            ),
            device=eedi3_device,
            opt=eedi3_opt,
        )
